...
} from "./agents/livekit-voice-agent.js";
...
/** PERFORMANCE: Fetch with timeout to prevent hanging requests */
...
interface DDGInfoboxItem {
	label: string;
	value: string;
}

// MCP Catalog - Progressive Discovery & Skill Distillation (927 servers, 13k+ tools)
import {
...
/**
 * Exa Web Search - Search the web using Exa MCP (no API key required)
 */
export function createWebSearchTool(): AgentTool<typeof webSearchSchema> {
	return {
		...
		label: "web_search",
		description:
			"Search the web using Exa AI MCP. Returns relevant web pages with titles, URLs, and content snippets. No API key required.",
		parameters: webSearchSchema,
		execute: async (_toolCallId, { query, numResults = 5, label }) => {
			logMcpTool("web_search", label);

			try {
				// Use Exa MCP HTTP endpoint (no API key needed)
						...
						if (line.startsWith("data:")) {
							...
							if (jsonStr) {
								try {
									...
									const content = parsed.result?.content?.[0]?.text || JSON.stringify(parsed.result, null, 2);
									return {
...
/**
 * Deep Research - AI-powered comprehensive research via Exa MCP
 */
export function createDeepResearchTool(): AgentTool<typeof deepResearchSchema> {
	return {
		...
		label: "deep_research",
		description:
			"Start a deep AI-powered research task using Exa MCP. Analyzes multiple sources and synthesizes findings. Best for complex questions requiring comprehensive analysis. No API key required.",
		parameters: deepResearchSchema,
		execute: async (_toolCallId, { question, label }) => {
			logMcpTool("deep_research", label);

			try {
				// Start research task via Exa MCP
				const startResponse = await fetch(
					"https://mcp.exa.ai/mcp?tools=deep_researcher_start,deep_researcher_check",
					{
						method: "POST",
						headers: {
							"Content-Type": "application/json",
				...
				// Parse SSE or JSON response to get taskId
				if (startText.includes("data:")) {
					for (const line of startText.split("\n")) {
						if (line.startsWith("data:")) {
							try {
								const parsed = JSON.parse(line.slice(5).trim());
								taskId = parsed.result?.content?.[0]?.text?.match(
									/task[_-]?id[:\s]*["']?([a-zA-Z0-9_-]+)/i,
								)?.[1];
								if (!taskId) taskId = parsed.result?.taskId || parsed.taskId;
							} catch {}
						}
					}
				} else {
					const data = JSON.parse(startText);
					taskId = data.result?.taskId || data.taskId;
				}

				if (!taskId) {
					return {
						content: [{ type: "text", text: "Research started but no task ID returned. Try again." }],
						details: undefined,
					};
				}
				...
				for (let i = 0; i < 18; i++) {
					...
					const checkResponse = await fetch("https://mcp.exa.ai/mcp?tools=deep_researcher_check", {
						method: "POST",
						headers: {
							"Content-Type": "application/json",
							Accept: "application/json, text/event-stream",
					...
					let result: { status?: string; report?: string; sources?: ResearchSource[] } | undefined;

					if (checkText.includes("data:")) {
						...
						result = JSON.parse(checkText).result;
					}
...
export function createWebScrapeTool(): AgentTool<typeof webScrapeSchema> {
	return {
		...
		execute: async (_toolCallId, { url, label }) => {
			...
			// Direct fetch fallback with timeout
			try {
				const response = await fetchWithTimeout(
					url,
					{
				...
				return { content: [{ type: "text", text: `Scrape failed: ${errMsg}` }], details: undefined };
			}
		},
	};
}
	...
	return {
		...
		execute: async (_toolCallId, { query, limit = 5, label }) => {
			...
			try {
				...
				const datasets = (await response.json()) as HFDataset[];
				const results =
					datasets
						.map(
							(d) =>
		...
		execute: async (_toolCallId, { query, label }) => {
			...
			try {
				...
				if (data.RelatedTopics && data.RelatedTopics.length > 0) {
					const ddgTopics = data.RelatedTopics as DDGTopic[];
					const topics = ddgTopics
						.filter((t) => t.Text)
						.slice(0, 5)
						.map((t) => `- ${t.Text}`);
					if (topics.length > 0) {
				...
				if (data.Infobox?.content) {
					const infoItems = data.Infobox.content as DDGInfoboxItem[];
	...
	return {
		...
		execute: async (_toolCallId, { path, lines = 100, label }) => {
			...
			try {
				...
				return { content: [{ type: "text", text: `Failed to read: ${getErrorMessage(err)}` }], details: undefined };
			}
		},
	};
}
	...
	return {
		...
		execute: async (_toolCallId, { path, label }) => {
			...
			try {
				...
				return { content: [{ type: "text", text: `Failed to list: ${getErrorMessage(err)}` }], details: undefined };
			}
		},
	};
}
...
export function getAllMcpTools(): AgentTool<any>[] {
	return [
		...
		// E2B Cloud Sandbox
		createE2BSandboxTool(),
		createE2BSandboxManagerTool(),
		...
		// Mario-style Skills (token-efficient CLI wrappers)
		createSequentialThinkTool(),
			...
			try {
				...
				return {
					content: [
						{
							type: "text",
							text: `Sub-agent failed: ${getErrorMessage(err)}`,
						},
					],
					details: undefined,
				};
...
// ============================================
// E2B Sandbox Tools - Isolated Cloud Execution
// ============================================

import { getE2BSandboxService, isE2BAvailable, runInSandbox } from "./agents/e2b-sandbox.js";

const e2bSandboxSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
	code: Type.String({ description: "Code to execute in the sandbox" }),
	language: Type.Optional(Type.String({ description: "Language: python, node, bash (default: bash)" })),
	timeout: Type.Optional(Type.Number({ description: "Sandbox timeout in seconds (default: 300)" })),
	keepAlive: Type.Optional(Type.Boolean({ description: "Keep sandbox alive after execution (default: false)" })),
});

/**
 * E2B Sandbox - Execute code in isolated cloud environment
 */
export function createE2BSandboxTool(): AgentTool<typeof e2bSandboxSchema> {
	return {
		name: "e2b_sandbox",
		label: "e2b_sandbox",
		description:
			"Execute code in an isolated E2B cloud sandbox. Safer than local execution. Supports Python, Node.js, Bash. Has Vue+FastAPI+SQLite stack available.",
		parameters: e2bSandboxSchema,
		execute: async (_toolCallId, { code, language = "bash", timeout = 300, keepAlive = false, label }) => {
			logMcpTool("e2b_sandbox", label);

			if (!isE2BAvailable()) {
				return {
					content: [{ type: "text", text: "E2B not configured. Set E2B_API_KEY environment variable." }],
					details: undefined,
				};
			}
			...
			try {
				const result = await runInSandbox(code, {
					language: language as "python" | "node" | "bash",
					timeout,
					keepAlive,
				});

				let output = `**E2B Sandbox (${language}):**\n`;
				if (result.sandboxId) {
					output += `Sandbox ID: \`${result.sandboxId}\`\n`;
				}
				...
				return {
					content: [{ type: "text", text: `E2B sandbox error: ${getErrorMessage(error)}` }],
					details: undefined,
				};
			}
		},
...
const e2bSandboxManageSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
	action: Type.String({
		description: "Action: create, exec, kill, info, host, write, read, list",
	}),
	sandboxId: Type.Optional(Type.String({ description: "Sandbox ID (required for most actions)" })),
	command: Type.Optional(Type.String({ description: "Command to execute (for exec action)" })),
	path: Type.Optional(Type.String({ description: "File path (for write/read actions)" })),
	content: Type.Optional(Type.String({ description: "File content (for write action)" })),
	port: Type.Optional(Type.Number({ description: "Port number (for host action, default: 5173)" })),
	timeout: Type.Optional(Type.Number({ description: "Timeout in seconds" })),
});

/**
 * E2B Sandbox Manager - Advanced sandbox operations
 */
export function createE2BSandboxManagerTool(): AgentTool<typeof e2bSandboxManageSchema> {
	return {
		name: "e2b_sandbox_manage",
		label: "e2b_sandbox_manage",
		description:
			"Manage E2B sandboxes: create, execute commands, read/write files, get public URLs, kill. Use for long-running tasks or hosting apps.",
		parameters: e2bSandboxManageSchema,
		execute: async (
			_toolCallId,
			{ action, sandboxId, command, path, content, port = 5173, timeout = 3600, label },
		) => {
			logMcpTool("e2b_sandbox_manage", label);
			...
			try {
				...
				switch (action.toLowerCase()) {
					case "create":
						result = await service.create({ timeout });
					...
					case "exec":
						if (!sandboxId || !command) {
							return {
								content: [{ type: "text", text: "Error: sandboxId and command required for exec" }],
								details: undefined,
							};
						}
						result = await service.exec(sandboxId, command, { shell: true, timeout: 120 });
						output = `**Exec Result:**\n\`\`\`\n${result.output || "(no output)"}\n\`\`\``;
						if (result.error) output += `\n**Error:** ${result.error}`;
						break;

					case "write":
						if (!sandboxId || !path || !content) {
							return {
								content: [{ type: "text", text: "Error: sandboxId, path, and content required for write" }],
								details: undefined,
							};
						}
						result = await service.writeFile(sandboxId, path, content);
						output = result.success ? `**File Written:** ${path}` : `**Error:** ${result.error}`;
						break;

					case "read":
						if (!sandboxId || !path) {
							return {
								content: [{ type: "text", text: "Error: sandboxId and path required for read" }],
								details: undefined,
							};
						}
						result = await service.readFile(sandboxId, path);
						output = result.success
							? `**${path}:**\n\`\`\`\n${result.output}\n\`\`\``
							: `**Error:** ${result.error}`;
						break;
					...
					case "list":
						if (!sandboxId) {
							return {
								content: [{ type: "text", text: "Error: sandboxId required for list" }],
								details: undefined,
							};
						}
						result = await service.listDir(sandboxId, path || "/home/user");
						output = result.success
							? `**Files:**\n\`\`\`\n${result.output}\n\`\`\``
							: `**Error:** ${result.error}`;
						break;
					...
					case "host":
						if (!sandboxId) {
							return {
								content: [{ type: "text", text: "Error: sandboxId required for host" }],
								details: undefined,
							};
						}
						result = await service.getHost(sandboxId, port);
						output = result.success ? `**Public URL:**\n${result.output}` : `**Error:** ${result.error}`;
						break;

					case "info":
						if (!sandboxId) {
							return {
								content: [{ type: "text", text: "Error: sandboxId required for info" }],
								details: undefined,
							};
						}
						result = await service.info(sandboxId);
						output = result.success ? `**Sandbox Info:**\n${result.output}` : `**Error:** ${result.error}`;
						break;

					case "kill":
						if (!sandboxId) {
							return {
								content: [{ type: "text", text: "Error: sandboxId required for kill" }],
								details: undefined,
							};
						}
						result = await service.kill(sandboxId);
						output = result.success ? `**Sandbox Killed:** ${sandboxId}` : `**Error:** ${result.error}`;
						break;

					default:
						output = `Unknown action: ${action}. Use: create, exec, write, read, list, host, info, kill`;
				}

				return { content: [{ type: "text", text: output }], details: undefined };
			} catch (error) {
				return {
					content: [{ type: "text", text: `E2B error: ${getErrorMessage(error)}` }],
					details: undefined,
				};
			}
		},
				...
				if (result.request_id) {
					...
					while (attempts < maxAttempts) {
						...
						const statusResponse = await fetch(
							`https://fal.run/${modelId}/requests/${result.request_id}/status`,
							{
						...
						if (status.status === "COMPLETED") {
							const resultResponse = await fetch(`https://fal.run/${modelId}/requests/${result.request_id}`, {
								headers: { Authorization: `Key ${FAL_KEY}` },
							});
	...
	return {
		...
		execute: async (_toolCallId, { prompt, outputFormat = "glb", guidanceScale = 15, label }) => {
			...
			try {
				...
				const response = await fetch("https://router.huggingface.co/hf-inference/models/openai/shap-e", {
					method: "POST",
					headers: {
						Authorization: `Bearer ${HF_TOKEN}`,
						"Content-Type": "application/json",
	...
	return {
		...
		execute: async (_toolCallId, { text, mediaPath, replyTo, label }) => {
			...
			try {
				...
				const _TWITTER_API_SECRET = process.env._TWITTER_API_SECRET;
				const _TWITTER_ACCESS_TOKEN = process.env._TWITTER_ACCESS_TOKEN;
				const _TWITTER_ACCESS_SECRET = process.env.TWITTER_ACCESS_TOKEN_SECRET;
		...
		execute: async (_toolCallId, { code, timeout = 30, packages = [], label }) => {
			...
			try {
				...
				try {
					...
					await fs.unlink(scriptPath).catch(() => {});
					const stderr = (e as { stderr?: string }).stderr || "none";
					return {
						content: [
							{
								type: "text",
								text: `Python execution error: ${getErrorMessage(e)}\n\n**Stderr:** ${stderr}`,
							},
						],
						details: undefined,
					};
	...
	return {
		...
		execute: async (_toolCallId, { action, schedule, tool, params, taskId, label }) => {
			...
			try {
				...
				let scheduled: Array<{ id: string; schedule: string; tool: string; params: unknown; createdAt: string }> =
					[];
				try {
...
const liveKitAgentSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
	action: Type.String({ description: "Action: join, speak, status, leave, start" }),
	...
	mode: Type.Optional(Type.String({ description: "Agent mode for start action: general, trading, coding, research" })),
});
...
export function createLiveKitAgentTool(): AgentTool<typeof liveKitAgentSchema> {
	return {
		...
		label: "livekit_agent",
		description:
			"AI agent for LiveKit rooms. Join rooms, speak with TTS, provide real-time voice AI. Supports start action for full voice AI agent with function tools.",
		parameters: liveKitAgentSchema,
		execute: async (_toolCallId, { action, roomName, text, voice = "alloy", mode = "general", label }) => {
			...
			try {
				...
				switch (action) {
					case "start": {
						// Start full voice AI agent with Python LiveKit Agents framework
						const agentMode = (mode || "general") as VoiceAgentMode;
						const agentVoice = voice as VoiceAgentVoice;
						...
						if (result.success) {
							return {
								content: [
									{
										type: "text",
										text: `🤖 **Voice AI Agent Started!**\n\n**Room:** ${result.roomName}\n**Mode:** ${result.mode}\n**Voice:** ${result.voice}\n**PID:** ${result.pid}\n\nThe agent is now active in the room with function tools (crypto prices, sentiment analysis).`,
									},
								],
								details: undefined,
							};
							...
							return {
								content: [
									{
										type: "text",
										text: `❌ **Failed to start voice agent**\n\n${result.error}\n\nMake sure LiveKit agents is installed: \`pip install -r requirements-livekit.txt\``,
									},
								],
								details: undefined,
							};
					...
					case "status": {
						...
						const voiceAgent = runningAgents.find((a) => a.roomName === roomName);
						...
						if (voiceAgent) {
							return {
								content: [
									{
										type: "text",
										text: `🤖 **Voice AI Agent Active**\n\n**Room:** ${voiceAgent.roomName}\n**Mode:** ${voiceAgent.mode}\n**Voice:** ${voiceAgent.voice}\n**PID:** ${voiceAgent.pid}\n**Uptime:** ${voiceAgent.uptimeSeconds}s`,
									},
								],
								details: undefined,
							};
					...
					case "leave": {
						...
						const stopped = stopVoiceAgentByRoom(roomName);

						delete state[roomName];
						await fs.writeFile(statePath, JSON.stringify(state, null, 2));

						const message = stopped
							? `✓ Voice AI agent stopped and left: ${roomName}`
							: `✓ Agent left: ${roomName}`;
...
// =============================================================================
// Sequential Thinking - Advanced Reasoning Chains (Mario-style skill wrapper)
// =============================================================================

const sequentialThinkSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
	action: Type.String({
		description: "Action: start, continue, revise, branch, hypothesis, verify, status, list",
	}),
	problem: Type.Optional(Type.String({ description: "Problem to analyze (for start action)" })),
	chainId: Type.Optional(
		Type.String({ description: "Chain ID (for continue/revise/branch/hypothesis/verify/status)" }),
	),
	thought: Type.Optional(Type.String({ description: "Thought content (for continue/revise/branch)" })),
	thoughtNumber: Type.Optional(Type.Number({ description: "Thought number to revise/branch from" })),
});

/**
 * Sequential Thinking - Advanced reasoning chains
 */
export function createSequentialThinkTool(): AgentTool<typeof sequentialThinkSchema> {
	return {
		name: "sequential_think",
		label: "sequential_think",
		description:
			"Advanced reasoning chains for complex problem-solving. Supports revision, branching, hypothesis testing. Use for multi-step analysis, planning with revision, and problems where scope is unclear.",
		parameters: sequentialThinkSchema,
		execute: async (_toolCallId, { action, problem, chainId, thought, thoughtNumber, label }) => {
			logMcpTool("sequential_think", label);

			try {
				const { execSync } = await import("child_process");
				const thinkScript = `${process.env.HOME}/.claude/skills/sequential-thinking/think.js`;
				...
				switch (action) {
					case "start":
						if (!problem)
							return {
								content: [{ type: "text", text: "Error: problem required for start" }],
								details: undefined,
							};
						cmd = `"${thinkScript}" "${problem.replace(/"/g, '\\"')}"`;
						break;
					case "continue":
						if (!chainId || !thought)
							return {
								content: [{ type: "text", text: "Error: chainId and thought required" }],
								details: undefined,
							};
						cmd = `"${thinkScript}" --continue ${chainId} "${thought.replace(/"/g, '\\"')}"`;
						break;
					case "revise":
						if (!chainId || !thoughtNumber || !thought)
							return {
								content: [{ type: "text", text: "Error: chainId, thoughtNumber, thought required" }],
								details: undefined,
							};
						cmd = `"${thinkScript}" --revise ${chainId} ${thoughtNumber} "${thought.replace(/"/g, '\\"')}"`;
						break;
					case "branch":
						if (!chainId || !thoughtNumber || !thought)
							return {
						...
						cmd = `"${thinkScript}" --branch ${chainId} ${thoughtNumber} "${thought.replace(/"/g, '\\"')}"`;
						break;
					case "hypothesis":
						if (!chainId || !thought)
							return {
								content: [{ type: "text", text: "Error: chainId and hypothesis required" }],
								details: undefined,
							};
						cmd = `"${thinkScript}" --hypothesis ${chainId} "${thought.replace(/"/g, '\\"')}"`;
						break;
					case "verify":
						if (!chainId || !thought)
							return {
								content: [{ type: "text", text: "Error: chainId and verification required" }],
								details: undefined,
							};
						cmd = `"${thinkScript}" --verify ${chainId} "${thought.replace(/"/g, '\\"')}"`;
						break;
					case "status":
						if (!chainId)
							return { content: [{ type: "text", text: "Error: chainId required" }], details: undefined };
						cmd = `"${thinkScript}" --status ${chainId}`;
						break;
					case "list":
						cmd = `"${thinkScript}" --list`;
						break;
					default:
						return { content: [{ type: "text", text: `Unknown action: ${action}` }], details: undefined };
				}
				...
				let output = `**Sequential Thinking - ${action.toUpperCase()}**\n\n`;
				...
				if (action === "list") {
					output +=
						parsed.length > 0
							? parsed
									.map(
										(c: { chainId: string; problem: string; status: string; thoughts: number }) =>
											`• **${c.chainId}**: ${c.problem} (${c.status}, ${c.thoughts} thoughts)`,
									)
									.join("\n")
							: "No thinking chains found.";
				} else if (action === "status") {
					output += `**Chain:** ${parsed.chainId}\n`;
					output += `**Problem:** ${parsed.problem}\n`;
					output += `**Status:** ${parsed.status}\n`;
					output += `**Thoughts:** ${parsed.totalThoughts}\n`;
					output += `**Branches:** ${parsed.branches}\n`;
					if (parsed.hypothesis) output += `**Hypothesis:** ${parsed.hypothesis}\n`;
					output += `**Verified:** ${parsed.verified}\n\n`;
					output += `**Thought Summary:**\n${parsed.thoughts
						.map(
							(t: { number: number; summary: string; isRevision: boolean }) =>
								`${t.number}. ${t.summary}${t.isRevision ? " (revision)" : ""}`,
						)
						.join("\n")}`;
				} else {
					output += `**Chain ID:** ${parsed.chainId}\n`;
					if (parsed.branchId) output += `**Branch:** ${parsed.branchId}\n`;
					output += `**Thought:** ${parsed.thoughtNumber}/${parsed.totalThoughts}\n`;
					output += `**Content:** ${parsed.thought || parsed.hypothesis || thought}\n`;
					if (parsed.isRevision) output += `**(Revises thought ${parsed.revisesThought})**\n`;
					output += `**Next needed:** ${parsed.nextThoughtNeeded}\n`;
					if (parsed.message) output += `\n${parsed.message}`;
				}

				return { content: [{ type: "text", text: output }], details: undefined };
			} catch (error) {
				return { content: [{ type: "text", text: `Sequential thinking error: ${error}` }], details: undefined };
			}
		},
	};
}
...
// =============================================================================
// Playwright Browser Automation (Mario-style skill wrapper)
// =============================================================================

const playwrightSchema = Type.Object({
	label: Type.String({ description: "Brief description (shown to user)" }),
	action: Type.String({
		description: "Action: screenshot, pdf, content, click, fill, eval, wait, extract, workflow",
	}),
	url: Type.String({ description: "URL to navigate to" }),
	selector: Type.Optional(Type.String({ description: "CSS selector (for click/wait/extract)" })),
	output: Type.Optional(Type.String({ description: "Output path (for screenshot/pdf)" })),
	data: Type.Optional(Type.String({ description: 'JSON data for fill action: {"#selector": "value"}' })),
	submit: Type.Optional(Type.String({ description: "Submit button selector (for fill action)" })),
	script: Type.Optional(Type.String({ description: "JavaScript to execute (for eval action)" })),
	selectors: Type.Optional(Type.String({ description: 'JSON selectors for extract: {"key": "selector"}' })),
	steps: Type.Optional(Type.String({ description: "JSON workflow steps array" })),
	format: Type.Optional(Type.String({ description: "Output format: markdown, html, text (for content)" })),
	device: Type.Optional(Type.String({ description: "Device to emulate: iPhone 12, Pixel 5, etc." })),
});

/**
 * Playwright - Advanced browser automation
 */
export function createPlaywrightTool(): AgentTool<typeof playwrightSchema> {
	return {
		name: "playwright",
		label: "playwright",
		description:
			"Advanced browser automation via Playwright. Screenshots, PDFs, form filling, data extraction, multi-step workflows. Supports Chromium, Firefox, WebKit. More reliable than Puppeteer.",
		parameters: playwrightSchema,
		execute: async (
			_toolCallId,
			{ action, url, selector, output, data, submit, script, selectors, steps, format, device, label },
		) => {
			logMcpTool("playwright", label);

			try {
				const { execSync } = await import("child_process");
				const browserScript = `${process.env.HOME}/.claude/skills/playwright/browser.js`;

				let cmd = `"${browserScript}" ${action} "${url}"`;
				...
				switch (action) {
					...
					case "pdf":
						if (!output)
							return {
								content: [{ type: "text", text: `Error: output path required for ${action}` }],
								details: undefined,
							};
						cmd += ` "${output}"`;
						break;
					...
					case "wait":
						if (!selector)
							return {
								content: [{ type: "text", text: `Error: selector required for ${action}` }],
								details: undefined,
							};
						cmd += ` "${selector}"`;
						break;
					case "fill":
						if (!data)
							return { content: [{ type: "text", text: "Error: data required for fill" }], details: undefined };
						cmd += ` --data '${data}'`;
						if (submit) cmd += ` --submit "${submit}"`;
						break;
					case "eval":
						if (!script)
							return {
								content: [{ type: "text", text: "Error: script required for eval" }],
								details: undefined,
							};
					...
					case "extract":
						if (!selectors)
							return {
								content: [{ type: "text", text: "Error: selectors required for extract" }],
								details: undefined,
							};
					...
					case "workflow":
						if (!steps)
							return {
								content: [{ type: "text", text: "Error: steps required for workflow" }],
								details: undefined,
							};
						cmd += ` --steps '${steps}'`;
						break;
				...
				// Format output
				let output_text = `**Playwright - ${action.toUpperCase()}**\n\n`;

				if (!parsed.success) {
					output_text += `**Error:** ${parsed.error}`;
				} else {
					output_text += `**URL:** ${parsed.url}\n`;

					switch (action) {
						case "screenshot":
						case "pdf":
							output_text += `**Saved to:** ${parsed.path}`;
							break;
						case "content":
							output_text += `**Title:** ${parsed.title}\n`;
							output_text += `**Format:** ${parsed.format}\n\n`;
							output_text += parsed.content.substring(0, 3000);
							if (parsed.content.length > 3000) output_text += "\n\n*(truncated)*";
							break;
						case "click":
							output_text += `**Clicked:** ${parsed.clicked}`;
							break;
						case "fill":
							output_text += `**Filled:** ${parsed.filled?.join(", ")}\n`;
							output_text += `**Submitted:** ${parsed.submitted}`;
							break;
						case "eval":
							output_text += `**Result:** ${JSON.stringify(parsed.result, null, 2)}`;
							break;
						case "wait":
							output_text += `**Found:** ${parsed.found}`;
							break;
						case "extract":
							output_text += `**Data:**\n\`\`\`json\n${JSON.stringify(parsed.data, null, 2)}\n\`\`\``;
							break;
						case "workflow":
							output_text += `**Steps completed:** ${parsed.steps?.length || 0}\n`;
							output_text += parsed.steps
								?.map(
									(s: { action: string; success: boolean; error?: string }, i: number) =>
										`${i + 1}. ${s.action}: ${s.success ? "✓" : `✗ ${s.error}`}`,
								)
								.join("\n");
							break;
					}
				...
				return { content: [{ type: "text", text: output_text }], details: undefined };
			} catch (error) {
				return { content: [{ type: "text", text: `Playwright error: ${error}` }], details: undefined };
			}
		},
	};
}
