...
/** CryptoPanic news item from API */
...
export class SentimentAgent extends BaseAgent {
	...
	private async analyzeSentiment(symbol: string): Promise<SentimentData | null> {
		...
		const sentimentScores = news.map((item) => this.calculateTitleSentiment(item.title));
		const avgScore = sentimentScores.reduce((a, b) => a + b, 0) / sentimentScores.length;

		// Extract keywords
		const keywords = this.extractKeywords(news.map((n) => n.title).join(" "));

		return {
			symbol,
			score: avgScore,
			volume: news.length,
			sources: [...new Set(news.map((n) => n.source?.title || "unknown"))],
			keywords,
			timestamp: Date.now(),
		};
	}

	private async fetchNews(symbol: string): Promise<CryptoPanicNewsItem[]> {
		try {
