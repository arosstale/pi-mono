...
/** Model vote result with optional error flag */
interface ModelVote {
	model: string;
...
export class ConsensusEngine {
	...
	async getConsensus(
		...
		const validResults = (results as ModelVote[]).filter((r) => !r.error);

		// Strip error flag for votes array (ConsensusResult.votes doesn't include error)
		const votes = (results as ModelVote[]).map(({ error: _, ...vote }) => vote);
		...
		const consensus = this.calculateConsensus(validResults, enabledModels);
