...
import { addExpression } from "./expressive-tts.js";
...
export class VibeVoiceTTS {
	...
	async synthesize(text: string, options: TTSOptions = {}): Promise<Buffer> {
		...
		// Apply expressive preprocessing if enabled
		let processedText = text;
		if (options.expressive) {
			const provider = this.useVibeVoice ? "vibevoice" : "mms-tts";
			processedText = addExpression(text, { provider });
		}
