...
/** Whisper API response segment */
interface WhisperSegment {
	avg_logprob?: number;
}

/** Whisper API verbose response */
...
export class GroqWhisperSTT {
	 ...
	 */
	private extractConfidence(data: WhisperVerboseResponse): number | undefined {
		if (data.segments && data.segments.length > 0) {
			// Average confidence across segments
			const totalConfidence = data.segments.reduce((sum, seg) => sum + (seg.avg_logprob || 0), 0);
