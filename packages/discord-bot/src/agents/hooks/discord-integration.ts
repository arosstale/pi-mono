...
export interface DiscordHookConfig {
	...
	expert?: boolean;
	/** Enable metrics hook (tool performance tracking) */
	metrics?: boolean;
...
export function createDiscordHookIntegration(config: DiscordHookConfig): HookIntegration {
	const { cwd, channelId, userId, checkpoint = true, lsp = true, expert = true, metrics = true } = config;
	...
	if (metrics) {
		const metricsHook = createMetricsHook({ enabled: true, serverName: `discord-${channelId}` });
		manager.register(
			createHookRegistration("metrics", metricsHook, {
				name: "Metrics",
				description: "Tool performance tracking",
			}),
		);
	}
