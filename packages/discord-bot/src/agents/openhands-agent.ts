...
const _DOCKER_IMAGE = "ghcr.io/all-hands-ai/openhands:main";
...
export async function runOpenHandsAgent(options: OpenHandsOptions): Promise<OpenHandsResult> {
	...
	// Options are passed through to runWithPython/runWithDocker/runWithAPI
	// Destructure only what we need here for status checks
	const { mode: _mode = "developer" } = options;
