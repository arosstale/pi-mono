...
/** Raw database row before transformation */
interface ArchivalEntryRow {
	id: number;
	agent_id: string;
	content: string;
	tags: string;
...
export class LocalTFIDFEmbedding implements EmbeddingProvider {
	...
	// biome-ignore lint/correctness/noUnusedPrivateClassMembers: Reserved for IDF recalculation
...
export class ArchivalMemory {
	...
	get(id: number): ArchivalEntry | null {
		...
		`,
			...
			.get(id) as ArchivalEntryRow | undefined;
