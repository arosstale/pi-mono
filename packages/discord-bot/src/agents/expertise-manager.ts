...
export function updateExpertise(
	...
	try {
		...
		// Truncate task description properly (only add ... if actually truncated)
		const maxTaskLen = 80;
		const taskDesc = task.length > maxTaskLen ? `${task.substring(0, maxTaskLen)}...` : task;

		// Clean learnings: remove excessive repetition and truncate if too long
		const maxLearningLen = 500;
		let cleanedLearnings = learnings
			.replace(/(.{20,}?)\1{3,}/g, "$1 [repeated]") // Remove repetitive content
			.trim();
		if (cleanedLearnings.length > maxLearningLen) {
			cleanedLearnings = `${cleanedLearnings.substring(0, maxLearningLen)}...`;
		}
		...
		if (current.includes(sessionMarker)) {
			// Append to existing session insights section
			const parts = current.split(sessionMarker);
			const header = parts[0];
			const insightsSection = parts[1] || "";

			// Parse existing insights by splitting on date-like headers
			const insightPattern = /\n\n### \d{4}-\d{2}-\d{2}/g;
			const matches = insightsSection.match(insightPattern) || [];

			// Count existing insights
			const existingCount = matches.length;

			if (existingCount >= maxInsights) {
				// Find and remove the oldest insight (first occurrence)
				const firstInsightIndex = insightsSection.search(/\n\n### \d{4}-\d{2}-\d{2}/);
				if (firstInsightIndex >= 0) {
					const secondInsightIndex = insightsSection.indexOf("\n\n### ", firstInsightIndex + 5);
					if (secondInsightIndex > firstInsightIndex) {
						// Remove first insight, keep the rest
						const trimmedInsights = insightsSection.substring(secondInsightIndex);
						updated = header + sessionMarker + trimmedInsights + newInsight;
					} else {
						// Only one insight exists, replace it
						updated = header + sessionMarker + newInsight;
					}
				} else {
					updated = header + sessionMarker + newInsight;
				}
				...
				// Just append
				updated = header + sessionMarker + insightsSection + newInsight;
			}
		} else {
