 ...
 *
 * This module exports a complete setup function that can be called from main.ts
 * to add all dashboard endpoints to the Express app.
 *
 * Usage in main.ts:
 *
 * 1. Add import:
 ...
 *
 * 2. After creating dashboardApp, before dashboardApp.listen(), add:
...
import type { Analytics, CommandMetrics, UserCost } from "./analytics.js";
import { dashboardHTML } from "./dashboard.js";
import type { BotStats, ModelInfo, ToolUsageStat } from "./dashboard-endpoints.js";
...
interface DashboardConfig {
	...
	channelStates: Map<string, unknown>;
	getToolUsageStats: () => ToolUsageStat[];
}

export function setupDashboard(app: Express, config: DashboardConfig): void {
	const { analytics, botStats, model, currentProvider, channelStates, getToolUsageStats } = config;
	...
	app.get("/api/costs", (_req: Request, res: Response) => {
		try {
			...
			res.json({
				...
				topUsers: topUsers.map((u: UserCost) => ({
					username: u.username,
	...
	app.get("/api/activity", (_req: Request, res: Response) => {
		try {
			...
			const activity = commandStats
				.filter((cmd: CommandMetrics) => cmd.lastUsed)
				.sort(
					(a: CommandMetrics, b: CommandMetrics) =>
						new Date(b.lastUsed).getTime() - new Date(a.lastUsed).getTime(),
				)
				.slice(0, 20)
				.map((cmd: CommandMetrics) => ({
