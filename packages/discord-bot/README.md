...
| `OTEL_SERVICE_NAME` | No | Service name for telemetry (default: pi-discord-bot) |
...
The bot includes optional OpenTelemetry distributed tracing and metrics for production monitoring.

### Features
- **Distributed Tracing**: Trace agent executions, tool calls, and message processing
- **Metrics**: Track tool execution rates, response times, and active sessions
- **Graceful Degradation**: No-op behavior when not configured (never crashes)
- **Production Ready**: Exports to Jaeger, Tempo, or any OTLP-compatible backend
...
### Available Metrics
- `agent.tool.executions` - Tool execution counts by status (success/error/timeout)
- `agent.response.time` - Response time histogram
- `agent.sessions.active` - Active agent sessions gauge

See [docs/observability-setup.md](docs/observability-setup.md) and [src/observability/README.md](src/observability/README.md) for full documentation.
